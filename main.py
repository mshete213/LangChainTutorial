...
reactPromptWithFormat = PromptTemplate(template=rawReactFormatInstructions, input_variables=["input", "agent_scratchpad", "tool_names"], output_parser=outputParser).partial(format_instructions=outputParser.get_format_instructions())
...
chain = agent_executor | extractOutput | parseOutput


def main():
    print("Hello from langchaintutorial!")

    result = chain.invoke({"input": "search for 3 job openings for an ai engineer using langchain in the bay area on linkedin and list their details."})
    print(result)
